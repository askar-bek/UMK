<?php
session_start();
require_once "db.php"; // должен быть mysqli $conn
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $discipline_volume = $_POST['discipline_volume'] ?? [];
    $instructions = $_POST['instructions'] ?? '';
    $plan_type = 'discipline_volume';

    // Формируем JSON для сохранения
    $plan_data = json_encode([
        'blocks' => $discipline_volume,
        'instructions' => $instructions,
        'plan_type' => $plan_type
    ], JSON_UNESCAPED_UNICODE);

    // Удалим старую запись для этого пользователя/типа (по желанию)
    $stmt = $conn->prepare("DELETE FROM UMK_userPlans WHERE user_id=? AND plan_type=?");
    $stmt->bind_param("is", $user_id, $plan_type);
    $stmt->execute();
    $stmt->close();

    // Вставим новую запись
    $stmt = $conn->prepare("INSERT INTO UMK_userPlans (user_id, plan_type, plan_data) VALUES (?, ?, ?)");
    $stmt->bind_param("iss", $user_id, $plan_type, $plan_data);
